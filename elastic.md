...
7. 过滤语句：**term**:用于精确匹配哪些值，比如数字，日期，布尔值或 not\_analyzed的字符串(未经分析的文本数据类型)。**terms**：terms 跟 term 有点类似，但 terms 允许指定多个匹配条件。 如果某个字段指定了多个值（**数组**），那么文档需要一起去做匹配。**range**：**gt/gte/lt/lte**。**exists/missing**:查找文档中是否包含指定字段或没有某个字段，类似于SQL语句中的IS_NULL条件
#####分布式CRUD  
